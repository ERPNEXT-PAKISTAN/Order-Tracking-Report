...
function loadItemsForGroup(frm) {
    ...
    if (byItem) {
        ...
        return Promise.resolve(frappe.call({
                method: "order_tracking_report.api.get_item_process_rows",
...
(function () {
  ...
  function loadDataEntryMasters() {
    ...
    var itemPromise = callApi("order_tracking_report.api.get_item_process_rows", {})
      .then(function (rows) {
  ...
  function loadSelectedItemProcessRows(selectedItem, forceRender) {
    ...
    callApi("order_tracking_report.api.get_item_process_rows", {
      item: itemName
    }).then(function (rows) {
  ...
  if (el("pp-sync-status-btn")) {
    el("pp-sync-status-btn").addEventListener("click", function () {
      ...
      callApi("order_tracking_report.api.force_sync_per_piece_status", {}).then(function (res) {
...
def _ensure_core_doctypes(results: list[str]) -> None:
	...
	fixture_path = frappe.get_app_path("order_tracking_report", "order_tracking_report", "fixtures", "doctype.json")
	try:
