(function () {
	...
	function renderTable(rows) {
		if (!rows || !rows.length) {
			return `<div style="padding:12px;color:#64748b;">No purchase orders found for this supplier in selected range.</div>`;
		}
	...
	async function loadSupplierStatus(frm, fromDate, toDate) {
		const field = frm.get_field("custom_supplier_purchase_status_html");
		if (!field || !field.$wrapper) return;
		field.$wrapper.find('[data-role="body"]').html(`<div style="padding:10px;color:#64748b;">Loading...</div>`);
		try {
			const r = await frappe.call({
				method: "order_tracking_report.api.get_supplier_purchase_order_status",
				args: {
	...
	frappe.ui.form.on("Supplier", {
		refresh(frm) {
			...
			if (!field || !field.$wrapper) return;
			if (frm.is_new()) {
				field.$wrapper.html(`<div style="padding:8px;color:#64748b;">Save Supplier to load purchase order status report.</div>`);
				return;
			}
			field.$wrapper.html(`
				<div style="border:1px solid #dbeafe;background:#f8fbff;border-radius:12px;padding:12px;">
					<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
						<div style="font-weight:800;color:#0f172a;">Purchase Order Status by Supplier</div>
			...
			field.$wrapper.find('[data-role="load"]').off("click").on("click", () => {
				loadSupplierStatus(frm, fromInput.val(), toInput.val());
			});
			loadSupplierStatus(frm, fromInput.val(), toInput.val());
		},
	});
