(function () {
  ...
  "after_row_insert": "purchase",
  "button_record_name": "Data Entry on Purchase Order",
  "doctype": "Purchase Order",
  "invalid_item_message": "Selected item is not marked as a purchase item",
  "item_filter_field": "is_purchase_item",
  "items_field": "items",
  "party_field": "supplier",
  "party_label": "Supplier",
  "party_options": "Supplier",
  "party_required": true,
  "rate_field": "rate",
  "rate_label": "Rate",
  "refresh_trigger": "calculate_taxes_and_totals",
  "script_name": "Data Entry on Purchase Order"
};
...
(function () {
	...
	function defaultFromDate() {
		return frappe.datetime.add_days(frappe.datetime.now_date(), -30);
	}
	function statusBadge(status) {
		const s = String(status || "").toLowerCase();
		let bg = "#e2e8f0", fg = "#334155";
		if (s.includes("complete") || s.includes("close")) { bg = "#dcfce7"; fg = "#166534"; }
		else if (s.includes("cancel")) { bg = "#fee2e2"; fg = "#991b1b"; }
		else if (s.includes("deliver") || s.includes("bill") || s.includes("receive")) { bg = "#fef3c7"; fg = "#92400e"; }
		return `<span style="display:inline-block;padding:2px 8px;border-radius:999px;background:${bg};color:${fg};font-weight:700;">${esc(status || "-")}</span>`;
	}
	function renderTable(rows) {
		if (!rows || !rows.length) {
			return `<div style="padding:12px;color:#64748b;">No rows found.</div>`;
		}
		const body = rows.map((r) => `
			<tr>
				<td style="padding:8px;border-bottom:1px solid #e2e8f0;">${esc(r.item || "")}</td>
		...
		return `
			<div style="overflow:auto;border:1px solid #e2e8f0;border-radius:10px;background:#fff;">
				<table style="width:100%;border-collapse:collapse;font-size:12px;min-width:720px;">
					<thead>
						<tr>
							<th style="padding:8px;text-align:left;background:#f8fafc;border-bottom:1px solid #e2e8f0;">Item</th>
							<th style="padding:8px;text-align:right;background:#f8fafc;border-bottom:1px solid #e2e8f0;">Rate</th>
							<th style="padding:8px;text-align:right;background:#f8fafc;border-bottom:1px solid #e2e8f0;">Order Qty</th>
							<th style="padding:8px;text-align:right;background:#f8fafc;border-bottom:1px solid #e2e8f0;">Received Qty</th>
							<th style="padding:8px;text-align:right;background:#f8fafc;border-bottom:1px solid #e2e8f0;">Pending Qty</th>
						</tr>
					</thead>
					<tbody>${body}</tbody>
				</table>
	...
	async function loadPoStatus(frm, fromDate, toDate) {
		const field = frm.get_field("custom_po_item_status_html");
		if (!field || !field.$wrapper) return;
		field.$wrapper.find('[data-role="po-status-body"]').html(`<div style="padding:10px;color:#64748b;">Loading...</div>`);
		try {
			const r = await frappe.call({
				method: "order_tracking_report.api.get_purchase_order_item_status",
				args: {
			...
			field.$wrapper.find('[data-role="po-status-meta"]').html(
				`<span style="margin-right:12px;">Status: ${statusBadge(meta.status || frm.doc.status || "")}</span>` +
				`<span style="margin-right:12px;">Supplier: <b>${esc(meta.supplier || frm.doc.supplier || "")}</b></span>` +
				`<span>Date: <b>${esc(meta.date || frm.doc.transaction_date || "")}</b></span>`
			);
			field.$wrapper.find('[data-role="po-status-body"]').html(renderTable(data.rows || []));
		} catch (e) {
			field.$wrapper.find('[data-role="po-status-body"]').html(`<div style="padding:10px;color:#b91c1c;">Failed to load report.</div>`);
		}
	}
	...
	frappe.ui.form.on("Purchase Order", {
		refresh(frm) {
			...
			if (!field || !field.$wrapper) return;
			if (frm.is_new()) {
				field.$wrapper.html(`<div style="padding:8px;color:#64748b;">Save Purchase Order to load item status report.</div>`);
				return;
			}
			field.$wrapper.html(`
				<div style="border:1px solid #dbeafe;background:#f8fbff;border-radius:12px;padding:12px;">
					<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
						<div style="font-weight:800;color:#0f172a;">Purchase Order Item Status</div>
						<div style="display:flex;gap:8px;align-items:end;">
							...
							<button class="btn btn-sm btn-primary" data-role="load-po-status">Load</button>
						</div>
					</div>
					<div data-role="po-status-meta" style="margin-top:8px;font-size:12px;color:#334155;"></div>
					<div data-role="po-status-body" style="margin-top:8px;"></div>
				</div>
			`);
			const fromInput = field.$wrapper.find('[data-role="from-date"]');
			const toInput = field.$wrapper.find('[data-role="to-date"]');
			fromInput.val(defaultFromDate());
			toInput.val(frappe.datetime.now_date());
			field.$wrapper.find('[data-role="load-po-status"]').off("click").on("click", () => {
				loadPoStatus(frm, fromInput.val(), toInput.val());
			});
			loadPoStatus(frm, fromInput.val(), toInput.val());
		},
	});
